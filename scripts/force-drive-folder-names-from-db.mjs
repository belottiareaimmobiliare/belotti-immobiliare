...
async function main() {
  ...
  console.log('')
  console.log('=== FORCE DRIVE FOLDER NAMES FROM DB ===')
  console.log({
  ...
  for (const row of rows) {
    const expectedName = String(row.folder_name || '').trim()
    const folderId = String(row.drive_folder_id || '').trim()
    const ref = String(row.properties?.reference_code || '').trim()
    ...
    try {
      ...
      const currentName = String(before?.folder?.name || '').trim()

      if (currentName === expectedName) {
        alreadyOk++
        console.log(`OK già allineata: ${expectedName}`)
        continue
      }

      console.log('')
      console.log(`RINOMINO: ${ref} | ${title}`)
      console.log(`  da: ${currentName}`)
      console.log(`  a : ${expectedName}`)
      ...
      console.log('')
      console.log(`ERRORE: ${expectedName}`)
      console.log(message)
    }
  }
...
main().catch((error) => {
  console.error('')
  console.error('FORCE RENAME DRIVE FALLITO:')
  console.error(error)
  process.exit(1)
})
