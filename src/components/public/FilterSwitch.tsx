...
export default function FilterSwitch({ checked, onChange, label }: Props) {
  return (
    <button
      ...
      className={`theme-pill liquid-button liquid-button-vertical flex w-full items-center justify-between gap-4 rounded-2xl border px-4 py-3 text-left text-sm transition ${
        checked ? 'liquid-button-selected' : ''
      }`}
    >
