  ...
  return (
    <button
      ...
      className={`theme-pill flex w-full items-center justify-between gap-4 rounded-2xl border px-4 py-3 text-left text-sm transition ${
        checked ? 'theme-pill-active' : ''
      }`}
    >
      <span
        className={`transition ${
          checked ? 'text-[var(--site-text)]' : 'text-[var(--site-text-soft)]'
        }`}
      >
        {label}
      </span>
      ...
      <span
        className={`relative inline-flex h-7 w-12 shrink-0 rounded-full border transition-all duration-300 ${
          checked
