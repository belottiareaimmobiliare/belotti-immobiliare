...
function folderLedClass(state: AIOSFolderVisualState) {
  if (state === 'green') return 'bg-[#A3BE8C] shadow-[0_0_14px_rgba(163,190,140,0.82)]'
  if (state === 'yellow') return 'bg-[#EBCB8B] shadow-[0_0_14px_rgba(235,203,139,0.82)]'
  if (state === 'red') return 'bg-[#BF616A] shadow-[0_0_14px_rgba(191,97,106,0.82)]'
  if (state === 'blue') return 'bg-[#88C0D0] shadow-[0_0_14px_rgba(136,192,208,0.80)]'
  return 'bg-[#4C566A] shadow-none'
}

function folderCardClass(state: AIOSFolderVisualState, isActive: boolean) {
  if (state === 'green') {
    return isActive
      ? 'border-[#A3BE8C]/45 bg-[#263528]/76 text-white shadow-[inset_0_0_0_1px_rgba(163,190,140,0.10),0_0_22px_rgba(163,190,140,0.10)]'
      : 'border-[#A3BE8C]/20 bg-[#222B28]/56 text-[#D8DEE9] hover:border-[#A3BE8C]/38 hover:bg-[#263528]/72'
  }

  if (state === 'yellow') {
    return isActive
      ? 'border-[#EBCB8B]/48 bg-[#343023]/76 text-white shadow-[inset_0_0_0_1px_rgba(235,203,139,0.10),0_0_22px_rgba(235,203,139,0.10)]'
      : 'border-[#EBCB8B]/20 bg-[#2D2A22]/56 text-[#D8DEE9] hover:border-[#EBCB8B]/38 hover:bg-[#343023]/72'
  }

  if (state === 'red') {
    return isActive
      ? 'border-[#BF616A]/50 bg-[#33252A]/78 text-white shadow-[inset_0_0_0_1px_rgba(191,97,106,0.10),0_0_22px_rgba(191,97,106,0.10)]'
      : 'border-[#BF616A]/22 bg-[#2D252B]/56 text-[#D8DEE9] hover:border-[#BF616A]/40 hover:bg-[#33252A]/72'
  }

  if (state === 'blue') {
    return isActive
      ? 'border-[#88C0D0]/45 bg-[#243241]/78 text-white shadow-[inset_0_0_0_1px_rgba(136,192,208,0.10),0_0_22px_rgba(136,192,208,0.10)]'
      : 'border-[#88C0D0]/18 bg-[#222B37]/56 text-[#D8DEE9] hover:border-[#88C0D0]/36 hover:bg-[#243241]/72'
  }

  return isActive
    ? 'border-[#8FBCBB]/35 bg-[#29313D]/78 text-white shadow-[inset_0_0_0_1px_rgba(143,188,187,0.08)]'
    : 'border-[#4C566A]/42 bg-[#202632]/62 text-[#D8DEE9] hover:border-[#8FBCBB]/28 hover:bg-[#252D3A]/78'
}
...
function agencyToolLedClass(toolId: AIOSAgencyToolId) {
  ...
  if (toolId === 'visure' || toolId === 'owner-docs' || toolId === 'visit') {
...
export default function AIOSDesktop() {
  ...
  const getFolderVisualState = (folder: AIOSFolder): AIOSFolderVisualState => {
    const isSelectedFolder = folder.id === activeFolderId
    ...
    const hasRedSignals =
      documentRequests.some((request) => request.status === 'cancelled') ||
      propertyMandates.some((mandate) => mandate.status === 'expired')

    if (hasRedSignals) return 'red'

    const hasWaitingSignals =
      documentRequests.some((request) => request.status === 'todo' || request.status === 'working') ||
      ownerDocuments.some((document) => document.status === 'missing' || document.status === 'received') ||
      checklistItems.some((item) => item.is_done === false)

    if (hasWaitingSignals) return 'yellow'

    const completedChecklistCount = checklistItems.filter((item) => item.is_done).length
    const hasStrongData =
      propertyOwners.length > 0 &&
      completedChecklistCount >= AI_OS_CHECKLIST_ITEMS.length &&
      ownerDocuments.length > 0 &&
      ownerDocuments.every((document) => document.status === 'verified' || document.status === 'not_needed')

    if (hasStrongData) return 'green'
    ...
    const hasSomeData =
      ...
      Number(folder.fileCount ?? folder.files.length ?? 0) > 0

    if (hasSomeData) return 'blue'
      ...
      <div className="mb-4 rounded-3xl border border-[#B48EAD]/25 bg-[#2E3440]/55 p-4 shadow-[0_18px_70px_rgba(0,0,0,0.30)]">
        ...
        {activeAgencyTool.id === 'visure' ? (
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-[#B48EAD]/18 bg-[#242B38]/82 p-4">
                ...
                <div className="grid gap-3">
                  {AI_OS_OWNER_DOCUMENTS.map((document) => {
                    ...
                    return (
                      <div
                        key={document.id}
                        className="rounded-2xl border border-[#8FBCBB]/12 bg-[#242B38]/76 p-3"
                      >
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                          <div>
  ...
  const renderSectionSwitcher = (mobile = false) => {
    ...
    return (
      <>
        <div className="mb-4 rounded-3xl border border-[#4C566A]/55 bg-[#252B36]/82 p-4 shadow-[0_18px_60px_rgba(46,52,64,0.32)]">
        <div className="mb-5">
              ...
              <button
                ...
                className={`group rounded-2xl border p-3 text-left transition active:scale-[0.99] ${
                  activeAgencyToolId === tool.id
                    ...
                    : 'border-[#8FBCBB]/12 bg-[#202632]/76 hover:border-[#B48EAD]/38 hover:bg-[#B48EAD]/10'
                }`}
              >
    ...
    <main className={`${jetBrainsMono.className} aios-nord fixed inset-0 z-[9999] h-dvh w-screen overflow-hidden bg-[#2E3440] text-[#D8DEE9] antialiased`}>
      ...
      <section className="relative z-10 hidden h-[calc(100dvh-56px)] w-full md:flex">
        ...
        <div className="relative min-w-0 flex-1 p-3 md:p-5">
          ...
          {desktopWindowOpen ? (
            <section className="h-[calc(100dvh-142px)] overflow-hidden rounded-[28px] border border-[#8FBCBB]/15 bg-[#202632]/88 shadow-2xl shadow-black/55 backdrop-blur-2xl">
              ...
              <div className="grid h-[calc(100%-53px)] grid-cols-1 overflow-hidden lg:grid-cols-[260px_minmax(0,1fr)_390px]">
                <aside className="border-b border-[#8FBCBB]/10 bg-[#1F2530]/72 p-4 lg:border-b-0 lg:border-r">
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <span className="relative flex h-7 w-7 shrink-0 items-center justify-center">
                                <span className={`absolute -right-0.5 top-0 h-2.5 w-2.5 rounded-full ring-2 ring-[#202632] ${folderLedClass(visualState)}`} />
                ...
                <section className="min-h-0 overflow-auto p-4">
                  ...
                  >
                    <div className="mb-4 rounded-2xl border border-[#8FBCBB]/10 bg-[#242B38]/78 p-4">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <label className="cursor-pointer rounded-full bg-[#A3BE8C] px-4 py-2 text-sm font-semibold text-[#D8DEE9] transition hover:bg-[#A3BE8C]/85">
                          Carica file
                          <input type="file" multiple accept={getUploadAccept(activeSection)} className="hidden" onChange={handleFileInput} />
                        </label>
