        ...
        {activeAgencyTool.id === 'visure' ? (
          ...
          <div className="rounded-2xl border border-[#8FBCBB]/15 bg-[#3B4252]/35 p-4">
            ...
            <p className="mt-2 text-lg font-semibold text-white">
              {activeFolder?.owner || 'Proprietario da completare'}
            </p>
            <p className="mt-2 text-xs leading-5 text-[#D8DEE9]/60">
              Qui metteremo anagrafica, recapiti, codice fiscale, ruolo, note riservate e comproprietari.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              {['Modifica anagrafica', 'Aggiungi comproprietario', 'Collega società'].map((action) => (
                <button
                  key={action}
                  type="button"
                  onClick={() => setNotice(`${action}: funzione pronta per il prossimo step database.`)}
                  className="rounded-full border border-[#8FBCBB]/25 bg-[#A3BE8C]/10 px-4 py-2 text-xs font-semibold text-[#E5E9F0] transition hover:bg-[#A3BE8C]/18"
                >
                  {action}
                </button>
              ))}
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {AI_OS_CHECKLIST_ITEMS.map((item) => (
              <div
                key={item}
                className="flex items-center gap-3 rounded-2xl border border-[#8FBCBB]/12 bg-[#3B4252]/32 px-3 py-2"
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-full border border-[#8FBCBB]/25 bg-[#2E3440]/65 text-xs text-[#88C0D0]">
                  □
                </span>
                <span className="text-sm text-[#E5E9F0]">{item}</span>
              </div>
            ))}
          </div>
        ) : (
